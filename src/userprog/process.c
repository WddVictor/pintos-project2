...
/*
resolve the command and its args
*/
char *extract_command(char *cmd, char *argv[], int *argc)
{
  char *cmd_copy = malloc(strlen(cmd) + 1);
  *argc = 0;
  char *save = NULL, *temp = NULL;
  strlcpy(cmd_copy, cmd, PGSIZE);

  temp = strtok_r(cmd_copy, " ", &save);
  argv[*argc] = temp;

  while ((temp = strtok_r(NULL, " ", &save)) != NULL)
    argv[++(*argc)] = temp;
